...
class ComputerBooksSpider(CrawlSpider):
    ...
    def get_size(self, size):
        if size:
            return size.split('／')[0].replace('サイズ ', '').replace('判', '').replace('変判', '')
        return size
    ...
    def parse_item(self, response):
        ...
        # loader.add_xpath('isbn', '//span[@itemprop="identifier"]/text()')
        loader.add_xpath('isbn', '//li[@itemprop="identifier"]/text()')
        loader.add_value('image_urls', [
            response.urljoin(response.xpath('//img[@itemprop="image"]/@src').get())
        ])
